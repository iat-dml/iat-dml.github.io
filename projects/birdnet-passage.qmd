...
::: {.callout-note}
**Coming soon.** The BirdNET-Passage codebase is being prepared for its first versioned release, which we expect very soon. Until then, the code is not yet publicly accessible. If you are interested in using it, please [get in touch](mailto:iat-dml@zalf.de) and we will let you know when it is available.
:::

BirdNET-Passage is research software that runs [BirdNET-Analyzer](https://github.com/birdnet-team/BirdNET-Analyzer) over very large collections of audio recordings. BirdNET is an AI model that recognises bird species from their calls and songs. BirdNET-Passage does not change the model. Instead, it makes sure that thousands of files pass through it quickly, and that every file is accounted for. We are developing it together with Marie Perennes and her group.
